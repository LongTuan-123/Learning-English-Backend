    ...
    const speaking = await SpeakingModel.find(
      ...
      .transform((docs) =>
        docs.map((doc) => ({
          ...
          audioQuestion: doc.AudioQuestion,
...
export const addSpeakingFile = async (req, res) => {
  try {
    const { question, audioQuestion, expiredTime, topicName } = req.body
...
export const updatePost = async (req, res) => {
  try {
    const { question, audioQuestion, expiredTime, topicName, speaking_id } = req.body
